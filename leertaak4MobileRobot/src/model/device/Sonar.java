...
public class Sonar extends Device {
    ...
    // JB: The use of the booleans detect and ping (and Device.running) makes the code very complex
    ...
    public void executeCommand(String command) {
        if (command.contains("ROTATETO")) {
            ...
            String measures = "PING";
            for (LaserMeasurement scanMeasure : pingMeasurements) {
                measure = scanMeasure;
    ...
    public void nextStep() {
        ...
        if (detect) {
            ...
            if (distance > -1.0) {
                pingMeasurements.add(new LaserMeasurement(distance, localPosition.getT()));  // ??????????????
            }
        }
    }
}
